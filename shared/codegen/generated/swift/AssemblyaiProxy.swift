...
//
//   let assemblyAIProxyErrorResponse = try AssemblyAIProxyErrorResponse(json)
...
// MARK: - AssemblyAIProxyErrorResponse
struct AssemblyAIProxyErrorResponse: Codable {
    ...
    /// codes.
    let error: AssemblyAIProxyErrorCode
    /// Present only alongside error=path_not_allowed; echoes the rejected subpath.
    let path: String?
}

// MARK: AssemblyAIProxyErrorResponse convenience initializers and mutators

extension AssemblyAIProxyErrorResponse {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(AssemblyAIProxyErrorResponse.self, from: data)
    }
    ...
    func with(
        error: AssemblyAIProxyErrorCode? = nil,
        path: String?? = nil
    ) -> AssemblyAIProxyErrorResponse {
        return AssemblyAIProxyErrorResponse(
...
enum AssemblyAIProxyErrorCode: String, Codable {
    case assemblyaiAPIKEYSecretNotSet = "ASSEMBLYAI_API_KEY secret not set"
