...
//
//   let stripePortalResponse = try StripePortalResponse(json)
//   let stripePortalErrorResponse = try StripePortalErrorResponse(json)

import Foundation

// MARK: - StripePortalResponse
struct StripePortalResponse: Codable {
    /// Stripe-hosted Billing Portal session URL.
    let url: String
}

// MARK: StripePortalResponse convenience initializers and mutators

extension StripePortalResponse {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(StripePortalResponse.self, from: data)
    }
...
// MARK: - StripePortalErrorResponse
struct StripePortalErrorResponse: Codable {
    ...
    /// codes.
    ///
    /// unauthenticated, no_subscription (no Stripe customer on file yet), or Stripe's own
    /// session.error.message on a Stripe API failure.
    let error: String
}

// MARK: StripePortalErrorResponse convenience initializers and mutators

extension StripePortalErrorResponse {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(StripePortalErrorResponse.self, from: data)
    }
    ...
    func with(
        error: String? = nil
    ) -> StripePortalErrorResponse {
        return StripePortalErrorResponse(
